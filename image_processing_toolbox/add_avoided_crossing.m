...
occupation_2 = calculate_occupation('fast', 0,...
...
if not(isempty(idx_i)) && not(isempty(idx_j))
...
for ii = 1:length(idx_i)
    ...
    if not(isempty(idx_trans)) && size(idx_trans, 1) > 1
        if idx_trans(1,1) > idx_trans(2,1)
            idx_trans = flipud(idx_trans);
        end
    end

    if not(isempty(idx_defect)) && size(idx_trans, 1) > 1
    ...
    if not(isempty(idx_defect)) && size(idx_trans, 1) > 1
        ...
        for j = 1:4
            ...
            for i = 1:size(Ai,1)
                ...
                i_intersect = max(1, round(i_intersect, 0));
                j_intersect = max(1, round(j_intersect, 0));   
                occ_antic(i_intersect, j_intersect) = 1;
            end
        end
